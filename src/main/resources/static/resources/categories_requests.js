...
let sendDeleteCategory = async () => {
    ...
    let errorMsg = 'Не можливо видалити категорію, коли вона використовується у транзакції. Спочатку треба видалити транзакцію.';
...
let sendDeleteTag = async () => {
    let id = getTagId();
    if (id === undefined) return;
    if (!confirm('Ви впевнені, що хочете видалити тег?')) return;
    let elem = tags.items.find(element => element.id === Number.parseInt(id));

    let jsonString = getJsonForSendingDelete(elem, 'tag');

    let errorMsg = 'Не можливо видалити тег, коли він використовується у транзакції. Спочатку треба видалити транзакцію.';
